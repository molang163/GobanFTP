...
Beta status as of 2026-05-25: the v1.1.0-beta.1 source line contains
fixture-backed repairs for the P0 subset below. Distribution creation, tag,
upload, and deploy remain maintainer release actions outside this roadmap entry.

P0 beta-fixed issues:
...
- WebDAV PROPFIND href spoof. Found in the v1.0.1 line: hrefs inside property
  ...
  entries. Beta repair: response-aware parsing admits only successful
...
- DNS owner-suffix parity gap. Found in the v1.0.1 line: scoped store admission
  and profile adapter witness admission could disagree. Beta repair:
...
- Local projection symlink escape. Found in the v1.0.1 line: projection writes
  could follow a preexisting projection-path symlink. Beta repair:
...
- Local event publish symlink escape. Found in the v1.0.1 line: local event
  publish could cross a preexisting event-directory symlink. Beta repair:
...
- FTP listing spoof. Found in the v1.0.1 line: compatibility listing parsing
  could be confused with exact publish confirmation. Beta repair: publish
...
- Malformed signed-HMAC attestations. Found in the v1.0.1 line: bad public
  JSONL could surface as an internal error. Beta repair: malformed
...
- CLI single-line diagnostic safety. Found in the v1.0.1 line: path-like values
  ...
  Beta repair: path-like CLI values reject ASCII control characters and
...
- FTP/WebDAV temporary publish name collision. Found in the v1.0.1 line:
  ...
  temporary publish name. Beta repair: temporary names include
  event-specific material.
- Invalid game descriptor roots. Found in the v1.0.1 line: consumers could be
  tempted to inspect root-like fields before validation. Beta repair:
...
- Input scale and timeout boundaries. Found in the v1.0.1 line: large WebDAV
  ...
  unbounded parsing. Beta repair: local fixture-tested body, href, line,
