...
fixtures, including real fixture `input_names` and unchanged event-set
preimage, root, replay, board, projection, and SGF truth. The next proof slice
should continue the claim audit and expand any remaining core/profile poison
coverage outside these public profile rows.
...
Git-tree and DNS-record runtime admissions and expand any remaining core/profile
public non-consensus poison coverage, not to add Git publish, live DNS, AXFR,
DNSSEC trust, provider APIs, dynamic update, DNS publish, hosted Web UI,
interactive TUI, or a v1.0/P14 completion claim.
